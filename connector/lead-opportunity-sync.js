...
async function authenticate(page){
 ...
 const status=await page.evaluate(async()=>{const r=await fetch('/api/data/v9.0/WhoAmI',{credentials:'include',headers:{Accept:'application/json'}});return r.status;});
...
async function fetchLeads(page,range,label,cfg){
 ...
 const url='/api/data/v9.0/'+entity+'?$select='+select+'&$filter='+encodeURIComponent(filter)+'&$expand=owningbusinessunit($select=name),customerid_contact($select=fullname,customertypecode,_ms_advisorid_value,_ms_marketeruserid_value)';
...
async function fetchOpps(page,range,label,cfg){
 ...
 const raw=await fetchPaged(page,label,'/api/data/v9.0/'+entity+'?$select='+select+'&$filter='+encodeURIComponent(filter)+'&$expand=createdby($select=fullname,_businessunitid_value)');
...
(async()=>{
 ...
 try{
  const cfg=await loadCrmConfig(connectorToken);CRM_ORIGIN=String(cfg.crmOrigin||CRM_ORIGIN).replace(/\/$/,'');
