...
function renderPeriod(period,pageId,title){
 ...
 $(pageId).innerHTML=`
  ...
  ${period==='daily'?'':`<div class="panel trend-main-panel"><div class="panel-head trend-panel-head"><div><h3>روند روزانه فعالیت</h3><p>نمودار با فیلترهای بالای صفحه همگام است؛ سه خط‌چین، میانگین راهنما، مشاور، و مشاور ارشد/سرتیم را بر اساس فهرست تیم مشاوران نشان می‌دهد.</p></div><div class="chart-select"><select id="${period}TrendMetric"><option value="talked">لید صحبت‌شده</option><option value="opp">فرصت OPP</option><option value="calls">تماس</option><option value="t8">T8</option><option value="tickets">تیکت</option><option value="total">فعالیت کل</option></select></div></div><div id="${period}TrendChart" class="line-chart">${trendChart(s.teamTrend||[],trendMetricState[period]||'total',s.roleTrend||{},s.roleCounts||{})}</div></div>`}
 ...
 if(period!=='daily'){
  ...
  if(sel){
   ...
   sel.onchange=()=>{trendMetricState[period]=sel.value;const cur=state.data[period]||{};$(period+'TrendChart').innerHTML=trendChart(cur.teamTrend||[],sel.value,cur.roleTrend||{},cur.roleCounts||{});bindTrendChartHover($(period+'TrendChart'));};
  }
