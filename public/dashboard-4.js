...
function activityCards(k,period){
 ...
 return `<div class="activity-kpis weekly-kpis"><div class="kpi"><div class="kpi-label">${title}</div><div class="kpi-value">${fa(k.total)}</div><div class="kpi-sub">لید صحبت‌شده + فرصت OPP + T8 + تیکت</div></div><div class="kpi"><div class="kpi-label">لید بسته‌شده</div><div class="kpi-value">${fa(k.closed)}</div><div class="kpi-sub">تعداد لیدهای بسته‌شده</div></div><div class="kpi"><div class="kpi-label">میانگین زمان بستن لید</div><div class="kpi-value">${k.closeAvg==null?'—':fa(Number(k.closeAvg).toFixed(1))}</div><div class="kpi-sub">روز؛ از تاریخ ثبت تا تاریخ آخرین تغییر</div></div><div class="kpi"><div class="kpi-label">لید صحبت‌شده</div><div class="kpi-value">${fa(k.talked)}</div><div class="kpi-sub">مبنای فعالیت</div></div><div class="kpi clickable" data-status-kpi="noStatus" data-period="${period}"><div class="kpi-label">نرخ عدم تعیین وضعیت در زمان مقرر</div><div class="kpi-value">${fa(Number(k.noStatusRate||0).toFixed(1))}٪</div><div class="kpi-sub">${fa(k.noStatusCount||0)} از ${fa(k.closed||0)} لید بسته‌شده</div></div><div class="kpi clickable" data-status-kpi="noResponse" data-period="${period}"><div class="kpi-label">نرخ عدم پاسخ (۲ بار)</div><div class="kpi-value">${fa(Number(k.noResponseRate||0).toFixed(1))}٪</div><div class="kpi-sub">${fa(k.noResponseCount||0)} از ${fa(k.closed||0)} لید رسیدگی‌شده</div></div><div class="kpi"><div class="kpi-label">فرصت از LEAD</div><div class="kpi-value">${fa(k.oppLead)}</div><div class="kpi-sub">صرفاً آماری</div></div><div class="kpi"><div class="kpi-label">فرصت از OPP</div><div class="kpi-value">${fa(k.opp)}</div><div class="kpi-sub">داخل فعالیت کل</div></div><div class="kpi"><div class="kpi-label">کل تماس</div><div class="kpi-value">${fa(k.calls)}</div><div class="kpi-sub">${fa(k.uniqueCalls)} لید یکتا / ${fa(k.repeatCalls)} تماس اضافه</div></div><div class="kpi"><div class="kpi-label">تماس ورودی T8</div><div class="kpi-value">${fa(k.t8)}</div><div class="kpi-sub">زیرمجموعه تماس‌ها</div></div><div class="kpi"><div class="kpi-label">تیکت</div><div class="kpi-value">${fa(k.tickets)}</div><div class="kpi-sub">رسیدگی‌شده</div></div>${period==='daily'?'':`<div class="kpi accent-kpi"><div class="kpi-label">سرانه تماس روزانه واحد</div><div class="kpi-value">${fa(Number(k.callAvg||0).toFixed(2))}</div></div><div class="kpi accent-kpi"><div class="kpi-label">سرانه لید بسته در روز</div><div class="kpi-value">${fa(Number(k.closedAvg||0).toFixed(2))}</div></div><div class="kpi accent-kpi"><div class="kpi-label">سرانه لید صحبت‌شده در روز</div><div class="kpi-value">${fa(Number(k.talkedAvg||0).toFixed(2))}</div></div>`}</div>`;
}

function renderPeriod(period,pageId,title){
 const s=state.data[period]||{},k=s.kpis||{},d=s.dimensions||{};
 const perPersonBenchmark=(filterState[period]?.advisor||[]).length===1;
 $(pageId).innerHTML=`
  ...
  ${period==='daily'?'':`<div class="panel trend-main-panel"><div class="panel-head trend-panel-head"><div><h3>روند روزانه فعالیت</h3><p>${perPersonBenchmark?'نمودار با فیلترهای بالای صفحه همگام است و خط‌چین، میانگین روزانه هر مشاور در کل واحد را نشان می‌دهد.':'نمودار با فیلترهای بالای صفحه همگام است و خط‌چین، میانگین روزانه کل واحد را نشان می‌دهد.'}</p></div><div class="chart-select"><select id="${period}TrendMetric"><option value="talked">لید صحبت‌شده</option><option value="opp">فرصت OPP</option><option value="calls">تماس</option><option value="t8">T8</option><option value="tickets">تیکت</option><option value="total">فعالیت کل</option></select></div></div><div id="${period}TrendChart" class="line-chart">${trendChart(s.teamTrend||[],trendMetricState[period]||'total',s.unitTrend||[],perPersonBenchmark?'perPerson':'unit',s.unitAdvisorCount||1)}</div></div>`}
 ...
 if(period!=='daily'){
  ...
  if(sel){
   ...
   sel.onchange=()=>{trendMetricState[period]=sel.value;const cur=state.data[period]||{},perPerson=(filterState[period]?.advisor||[]).length===1;$(period+'TrendChart').innerHTML=trendChart(cur.teamTrend||[],sel.value,cur.unitTrend||[],perPerson?'perPerson':'unit',cur.unitAdvisorCount||1);bindTrendChartHover($(period+'TrendChart'));};
  }
