...
export async function getFilteredPeriodSummary(period:string,filters:any={}){
 ...
 const q:any=await namedBatch([
  ...
  {key:'days',sql:`SELECT "day" FROM (SELECT date(last_modified_date) AS "day" FROM ${L}${lw.where} AND last_modified_date<>'' UNION SELECT date(created_date) AS "day" FROM ${O}${ow.where} AND created_date<>'' UNION SELECT date(start_date) AS "day" FROM ${C}${cw.where} AND start_date<>'' UNION SELECT date(closed_at) AS "day" FROM ${T}${tw.where} AND closed_at<>'') WHERE "day" IS NOT NULL ORDER BY "day"`,args:[...lw.args,...ow.args,...cw.args,...tw.args]},
  ...
  {key:'attendanceByAdvisor',sql:`SELECT name,COUNT(*) attendanceDays FROM (
    SELECT user name,date(start_date) AS "day"
    FROM ${C}${acw.where}
      AND COALESCE(user,'')<>''
      AND start_date<>''
  ...
  {key:'trendLeads',sql:`SELECT date(last_modified_date) AS "day",SUM(CASE WHEN COALESCE(last_status,'') NOT IN ${TALKED_EXCLUDED_SQL} THEN 1 ELSE 0 END) talked FROM ${L}${lw.where} AND last_modified_date<>'' GROUP BY "day"`,args:lw.args},
  {key:'trendOpps',sql:`SELECT date(created_date) AS "day",COUNT(*) opp FROM ${O}${ow.where} AND registration_type='OPP' AND created_date<>'' GROUP BY "day"`,args:ow.args},
  {key:'trendCalls',sql:`SELECT date(start_date) AS "day",COUNT(*) calls,SUM(CASE WHEN UPPER(COALESCE(queue,''))='T8' THEN 1 ELSE 0 END) t8 FROM ${C}${cw.where} AND start_date<>'' GROUP BY "day"`,args:cw.args},
  {key:'trendTickets',sql:`SELECT date(closed_at) AS "day",COUNT(*) tickets FROM ${T}${tw.where} AND closed_at<>'' GROUP BY "day"`,args:tw.args},
  {key:'teamTrendLead',sql:`SELECT date(last_modified_date) AS "day",SUM(CASE WHEN COALESCE(last_status,'') NOT IN ${TALKED_EXCLUDED_SQL} THEN 1 ELSE 0 END) talked FROM ${L}${lw.where} AND owner IN (SELECT name FROM team_members) AND last_modified_date<>'' GROUP BY "day"`,args:lw.args},
  {key:'teamTrendOpp',sql:`SELECT date(created_date) AS "day",COUNT(*) opp FROM ${O}${ow.where} AND creator IN (SELECT name FROM team_members) AND registration_type='OPP' AND created_date<>'' GROUP BY "day"`,args:ow.args},
  {key:'teamTrendCall',sql:`SELECT date(start_date) AS "day",COUNT(*) calls,SUM(CASE WHEN UPPER(COALESCE(queue,''))='T8' THEN 1 ELSE 0 END) t8 FROM ${C}${cw.where} AND user IN (SELECT name FROM team_members) AND start_date<>'' GROUP BY "day"`,args:cw.args},
  {key:'teamTrendTicket',sql:`SELECT date(closed_at) AS "day",COUNT(*) tickets FROM ${T}${tw.where} AND owner IN (SELECT name FROM team_members) AND closed_at<>'' GROUP BY "day"`,args:tw.args},
  {key:'unitTrendLead',sql:`SELECT date(last_modified_date) AS "day",SUM(CASE WHEN COALESCE(last_status,'') NOT IN ${TALKED_EXCLUDED_SQL} THEN 1 ELSE 0 END) talked FROM ${L}${blw.where} AND owner IN (SELECT name FROM team_members) AND last_modified_date<>'' GROUP BY "day"`,args:blw.args},
  {key:'unitTrendOpp',sql:`SELECT date(created_date) AS "day",COUNT(*) opp FROM ${O}${bow.where} AND creator IN (SELECT name FROM team_members) AND registration_type='OPP' AND created_date<>'' GROUP BY "day"`,args:bow.args},
  {key:'unitTrendCall',sql:`SELECT date(start_date) AS "day",COUNT(*) calls,SUM(CASE WHEN UPPER(COALESCE(queue,''))='T8' THEN 1 ELSE 0 END) t8 FROM ${C}${bcw.where} AND user IN (SELECT name FROM team_members) AND start_date<>'' GROUP BY "day"`,args:bcw.args},
  {key:'unitTrendTicket',sql:`SELECT date(closed_at) AS "day",COUNT(*) tickets FROM ${T}${btw.where} AND owner IN (SELECT name FROM team_members) AND closed_at<>'' GROUP BY "day"`,args:btw.args},
  {key:'roleTrendLead',sql:`SELECT date(l.last_modified_date) AS "day",CASE WHEN tm.role='راهنما' THEN 'guide' WHEN tm.role='مشاور' THEN 'advisor' WHEN tm.role IN ('مشاور ارشد','سرتیم') THEN 'senior' END roleGroup,SUM(CASE WHEN COALESCE(l.last_status,'') NOT IN ${TALKED_EXCLUDED_SQL} THEN 1 ELSE 0 END) talked FROM ${L} l JOIN team_members tm ON tm.name=l.owner ${blw.where} AND l.last_modified_date<>'' AND tm.role IN ('راهنما','مشاور','مشاور ارشد','سرتیم') GROUP BY "day",roleGroup`,args:blw.args},
  {key:'roleTrendOpp',sql:`SELECT date(o.created_date) AS "day",CASE WHEN tm.role='راهنما' THEN 'guide' WHEN tm.role='مشاور' THEN 'advisor' WHEN tm.role IN ('مشاور ارشد','سرتیم') THEN 'senior' END roleGroup,COUNT(*) opp FROM ${O} o JOIN team_members tm ON tm.name=o.creator ${bow.where} AND o.registration_type='OPP' AND o.created_date<>'' AND tm.role IN ('راهنما','مشاور','مشاور ارشد','سرتیم') GROUP BY "day",roleGroup`,args:bow.args},
  {key:'roleTrendCall',sql:`SELECT date(c.start_date) AS "day",CASE WHEN tm.role='راهنما' THEN 'guide' WHEN tm.role='مشاور' THEN 'advisor' WHEN tm.role IN ('مشاور ارشد','سرتیم') THEN 'senior' END roleGroup,COUNT(*) calls,SUM(CASE WHEN UPPER(COALESCE(c.queue,''))='T8' THEN 1 ELSE 0 END) t8 FROM ${C} c JOIN team_members tm ON tm.name=c.user ${bcw.where} AND c.start_date<>'' AND tm.role IN ('راهنما','مشاور','مشاور ارشد','سرتیم') GROUP BY "day",roleGroup`,args:bcw.args},
  {key:'roleTrendTicket',sql:`SELECT date(t.closed_at) AS "day",CASE WHEN tm.role='راهنما' THEN 'guide' WHEN tm.role='مشاور' THEN 'advisor' WHEN tm.role IN ('مشاور ارشد','سرتیم') THEN 'senior' END roleGroup,COUNT(*) tickets FROM ${T} t JOIN team_members tm ON tm.name=t.owner ${btw.where} AND t.closed_at<>'' AND tm.role IN ('راهنما','مشاور','مشاور ارشد','سرتیم') GROUP BY "day",roleGroup`,args:btw.args}
 ]);
