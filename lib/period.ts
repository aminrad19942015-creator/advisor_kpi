 ...
 const q:any=await namedBatch([
  ...
  {key:'attendanceByAdvisor',sql:`SELECT name,COUNT(*) AS "attendanceDays" FROM (
    ...
    FROM ${C}${acw.where}
      AND COALESCE(user,'')<>''
      AND start_date<>''
    GROUP BY user,(start_date::timestamptz)::date
    HAVING COUNT(*)>=5
  ...
  {key:'trendLeads',sql:`SELECT (last_modified_date::timestamptz)::date AS "day",SUM(CASE WHEN COALESCE(last_status,'') NOT IN ${TALKED_EXCLUDED_SQL} THEN 1 ELSE 0 END) talked FROM ${L}${lw.where} AND last_modified_date<>'' GROUP BY "day"`,args:lw.args},
  {key:'trendOpps',sql:`SELECT (created_date::timestamptz)::date AS "day",COUNT(*) opp FROM ${O}${ow.where} AND registration_type='OPP' AND created_date<>'' GROUP BY "day"`,args:ow.args},
  {key:'trendCalls',sql:`SELECT (start_date::timestamptz)::date AS "day",COUNT(*) calls,SUM(CASE WHEN UPPER(COALESCE(queue,''))='T8' THEN 1 ELSE 0 END) t8 FROM ${C}${cw.where} AND start_date<>'' GROUP BY "day"`,args:cw.args},
  {key:'trendTickets',sql:`SELECT (closed_at::timestamptz)::date AS "day",COUNT(*) tickets FROM ${T}${tw.where} AND closed_at<>'' GROUP BY "day"`,args:tw.args},
  {key:'teamTrendLead',sql:`SELECT (last_modified_date::timestamptz)::date AS "day",SUM(CASE WHEN COALESCE(last_status,'') NOT IN ${TALKED_EXCLUDED_SQL} THEN 1 ELSE 0 END) talked FROM ${L}${lw.where} AND owner IN (SELECT name FROM team_members) AND last_modified_date<>'' GROUP BY "day"`,args:lw.args},
  {key:'teamTrendOpp',sql:`SELECT (created_date::timestamptz)::date AS "day",COUNT(*) opp FROM ${O}${ow.where} AND creator IN (SELECT name FROM team_members) AND registration_type='OPP' AND created_date<>'' GROUP BY "day"`,args:ow.args},
  {key:'teamTrendCall',sql:`SELECT (start_date::timestamptz)::date AS "day",COUNT(*) calls,SUM(CASE WHEN UPPER(COALESCE(queue,''))='T8' THEN 1 ELSE 0 END) t8 FROM ${C}${cw.where} AND user IN (SELECT name FROM team_members) AND start_date<>'' GROUP BY "day"`,args:cw.args},
  {key:'teamTrendTicket',sql:`SELECT (closed_at::timestamptz)::date AS "day",COUNT(*) tickets FROM ${T}${tw.where} AND owner IN (SELECT name FROM team_members) AND closed_at<>'' GROUP BY "day"`,args:tw.args},
  {key:'unitTrendLead',sql:`SELECT (last_modified_date::timestamptz)::date AS "day",SUM(CASE WHEN COALESCE(last_status,'') NOT IN ${TALKED_EXCLUDED_SQL} THEN 1 ELSE 0 END) talked FROM ${L}${blw.where} AND owner IN (SELECT name FROM team_members) AND last_modified_date<>'' GROUP BY "day"`,args:blw.args},
  {key:'unitTrendOpp',sql:`SELECT (created_date::timestamptz)::date AS "day",COUNT(*) opp FROM ${O}${bow.where} AND creator IN (SELECT name FROM team_members) AND registration_type='OPP' AND created_date<>'' GROUP BY "day"`,args:bow.args},
