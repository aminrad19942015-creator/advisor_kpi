...
export async function getFilteredPeriodSummary(period:string,filters:any={}){
 ...
 const q:any=await namedBatch([
  ...
  {key:'roleTrendLead',sql:`SELECT (l.last_modified_date::timestamptz AT TIME ZONE 'Asia/Tehran')::date AS "day",CASE WHEN tm.role='راهنما' THEN 'guide' WHEN tm.role='مشاور' THEN 'advisor' WHEN tm.role IN ('مشاور ارشد','سرتیم') THEN 'senior' END AS "roleGroup",SUM(CASE WHEN COALESCE(l.last_status,'') NOT IN ${TALKED_EXCLUDED_SQL} THEN 1 ELSE 0 END) talked FROM ${L} l JOIN team_members tm ON tm.name=l.owner ${blw.where} AND l.last_modified_date<>'' AND tm.role IN ('راهنما','مشاور','مشاور ارشد','سرتیم') GROUP BY "day","roleGroup"`,args:blw.args},
