...
export async function getActivityDetails(period:string,type:string,user:string){let source:'lead'|'opp'|'call'|'ticket',where:string;if(type==='lead'||type==='talked'){source='lead';where='owner=?'+(type==='talked'?` AND COALESCE(last_status,'') NOT IN ${TALKED_EXCLUDED_SQL}`:'')}else if(type==='opp'){source='opp';where="creator=? AND registration_type='OPP'"}else if(type==='call'){source='call';where='user=?'}else if(type==='t8'){source='call';where="user=? AND UPPER(COALESCE(queue,''))='T8'"}else if(type==='ticket'){source='ticket';where='owner=?'}else return [];const select=source==='call'?`*,COALESCE(NULLIF(phone_number,''),destination_number) AS customer_phone`:'*';return tursoSelect(`SELECT ${select} FROM ${table(period,source)} WHERE ${where} LIMIT 4000`,[user]);}
