...
export async function getFilteredPeriodSummary(period:string,filters:any={}){
 ...
 const q:any=await namedBatch([
  ...
  {key:'roleTrendLead',sql:`SELECT date(l.last_modified_date) day,CASE WHEN tm.role='راهنما' THEN 'guide' WHEN tm.role='مشاور' THEN 'advisor' WHEN tm.role IN ('مشاور ارشد','سرتیم') THEN 'senior' END roleGroup,SUM(CASE WHEN COALESCE(l.last_status,'') NOT IN ${TALKED_EXCLUDED_SQL} THEN 1 ELSE 0 END) talked FROM ${L} l JOIN team_members tm ON tm.name=l.owner ${blw.where} AND l.last_modified_date<>'' AND tm.role IN ('راهنما','مشاور','مشاور ارشد','سرتیم') GROUP BY day,roleGroup`,args:blw.args},
  {key:'roleTrendOpp',sql:`SELECT date(o.created_date) day,CASE WHEN tm.role='راهنما' THEN 'guide' WHEN tm.role='مشاور' THEN 'advisor' WHEN tm.role IN ('مشاور ارشد','سرتیم') THEN 'senior' END roleGroup,COUNT(*) opp FROM ${O} o JOIN team_members tm ON tm.name=o.creator ${bow.where} AND o.registration_type='OPP' AND o.created_date<>'' AND tm.role IN ('راهنما','مشاور','مشاور ارشد','سرتیم') GROUP BY day,roleGroup`,args:bow.args},
  {key:'roleTrendCall',sql:`SELECT date(c.start_date) day,CASE WHEN tm.role='راهنما' THEN 'guide' WHEN tm.role='مشاور' THEN 'advisor' WHEN tm.role IN ('مشاور ارشد','سرتیم') THEN 'senior' END roleGroup,COUNT(*) calls,SUM(CASE WHEN UPPER(COALESCE(c.queue,''))='T8' THEN 1 ELSE 0 END) t8 FROM ${C} c JOIN team_members tm ON tm.name=c.user ${bcw.where} AND c.start_date<>'' AND tm.role IN ('راهنما','مشاور','مشاور ارشد','سرتیم') GROUP BY day,roleGroup`,args:bcw.args},
  {key:'roleTrendTicket',sql:`SELECT date(t.closed_at) day,CASE WHEN tm.role='راهنما' THEN 'guide' WHEN tm.role='مشاور' THEN 'advisor' WHEN tm.role IN ('مشاور ارشد','سرتیم') THEN 'senior' END roleGroup,COUNT(*) tickets FROM ${T} t JOIN team_members tm ON tm.name=t.owner ${btw.where} AND t.closed_at<>'' AND tm.role IN ('راهنما','مشاور','مشاور ارشد','سرتیم') GROUP BY day,roleGroup`,args:btw.args}
 ]);
